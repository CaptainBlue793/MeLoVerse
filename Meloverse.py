...
colored_text = ''.join(
    ['<span style="color: hsl({}, 60%, 50%);">{}</span>'.format(i * 360 / len(text), char) for i, char in
     enumerate(text)])
colored_text_with_malt = ' </span>  ' + colored_text + '  <span style="color: hsl(250, 70%, 70%);">&#x260A;</span>'
...
def add_gradient_bg():
    st.markdown(
        ...
        <style>
       .stApp {{
            background: linear-gradient(to bottom, #0D0D1A, #1F2143);
...
if st.button('Recommend'):
    ...
    st.markdown(
       ...
       .column-content:hover {
            ...
            background-color: #0D0D1A; /* Darker background on hover */
        }
       .column-image {
            display: flex;
            justify-content: center;
       ...
       .music-title {
            color: #F5FEFD;
            font-family: 'Open Sans', sans-serif; /* Use font-family for better compatibility */
            font-size: 15px;
            margin-bottom: 5px;
        }
        </style>
