...
colored_text = ''.join(
    ['<span style="color: hsl({}, 100%, 50%);">{}</span>'.format(i * 360 / len(text), char) for i, char in
     enumerate(text)])

st.markdown(f'<h1 class="meloverse-title">{colored_text}</h1>', unsafe_allow_html=True)
